...
const patchListData = (
  listData: Partial<LX.Sync.List.ListData>,
...
const mergeList = (
  ...
  const addMusicLocationType = getUserConfig(socket.userInfo.name)[
  ...
  newListData.defaultList = handleMergeList(
    sourceListData.defaultList,
    targetListData.defaultList,
    addMusicLocationType,
  )
  newListData.loveList = handleMergeList(
  ...
  targetListData.userList.forEach((list, index) => {
    ...
    if (sourceList) {
      sourceList.list = handleMergeList(
        sourceList.list,
        list.list,
        addMusicLocationType,
      )
  ...
  if (
    ...
    if (
      ...
      await setRemotelList(
        socket,
        localListData,
        await userSpace.listManage.getCurrentListInfoKey(),
      )
    }
  } else {
    let key: string
...
const handleMergeListDataFromSnapshot = async (
  ...
  newListData.defaultList = mergeListDataFromSnapshot(
    localListData.defaultList,
    remoteListData.defaultList,
    snapshot.defaultList,
    addMusicLocationType,
  )
  newListData.loveList = mergeListDataFromSnapshot(
    localListData.loveList,
    remoteListData.loveList,
    snapshot.loveList,
    addMusicLocationType,
  )
  ...
  const newUserList: LX.List.UserListInfoFull[] = []
  for (const list of localListData.userList) {
    ...
    if (remoteList) {
      ...
      newList = buildUserListInfoFull({
        ...
        sourceListId: selectData(
          snapshotList.sourceListId,
          list.sourceListId,
          remoteList.sourceListId,
        ),
        locationUpdateTime: list.locationUpdateTime,
        list: mergeListDataFromSnapshot(
          list.list,
          remoteList.list,
          snapshotList.list,
          addMusicLocationType,
  ...
  remoteListData.userList.forEach((list, index) => {
    ...
    if (localUserListData.has(list.id)) {
      const localUpdateTime =
        localUserListData.get(list.id)?.locationUpdateTime ?? 0
  ...
  await overwriteRemoteListData(socket, newListData, key, [
    socket.keyInfo.clientId,
  ])
  if (err) throw err
}
...
const syncList = async (socket: LX.Socket) => {
  ...
  if (!socket.feature.list.skipSnapshot) {
    ...
    if (userCurrentListInfoKey) {
      const listData = await user.listManage.snapshotDataManage.getSnapshot(
...
export const sync = async (socket: LX.Socket) => {
  const syncRef = socket.syncRefs.list
  let disconnected = false
  socket.onClose(() => {
    disconnected = true
    if (syncRef.current === socket.keyInfo.clientId) syncRef.current = null
  })
